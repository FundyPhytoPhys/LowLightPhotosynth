...
Within, and across, species of *Chlamydomonas* XXXXCITE Pomona PaperXXXX growth under higher salinities increases maintenance of F~V~/F~M~ and photosynthetic capacity during stress.  We therefore grew each *Chlamydomonas* strain across three growth-permissive salinities (TABLEXXX) and performed our saturating flash trains.  We ran an ANOVA for damping indices in relation to taxa,  effective equivalent light level (derived from the saturating flash spacing and 𝛔~PSII~), measurement temperature, and the growth concentration of NaCl, with interactions among all factors (Supp. Table XXXX).  As expected, taxa, effective equivalent light level and measurement temperature all showed strongly significant influences on damping indices.  NaCl concentration also showed a significant influence on damping indices across strains, interacting with effective equivalent light level.

We then ran the ANOVA for each taxa separately (Supp. Table XXX).  Within a taxa only *C. priscui* showed a significant effect of NaCl on damping indices, while *C. reinhardtii* showed an interactive effect from measurement temperature and NaCl.
