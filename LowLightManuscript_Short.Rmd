...
In parallel with seasonally low light, psychrophile phytoplankton contend with cold temperature constraints on water viscosity, solute diffusion rates, membrane fluidity, enzyme kinetics and macromolecule interactions [@lyonPolarMicroalgaeNew2014]. In parallel salinity varies across polar habitats  [REFERENCES MACKENZIE ET AL], along with local salinity fluctuations driven by freezing and thawing of ice [REFERENCES MACKENZIE ET AL].  XXXPOMONA ARTICLE(S) on SALINITY RESPONSES?XXX.
...
XXXREDO SUmmary TABLE TO INCLUDE SALINITYXXXX
```{r taxacultures, echo = FALSE}
Taxa <- c("Thalassiosira pseudonana", "Thalassiosira pseudonana", "Chlorella vulgaris", "Chlorella vulgaris",  "Chlamydomonas reinhardtii", "Chlamydomonas euryale",  "Fragilariopsis cylindrus",  "Chlamydomonas priscui", "Chlamydomonas sp. ICE-MDV", "Chlamydomonas malina", "Chlamydomonas klinobasis")

Lineage <- c(rep("Diatom", 2), rep("Green", 4), rep("Diatom", 1), rep("Green", 4))

Origin <- c(rep("Temperate", 6), rep("Polar", 5))
Growth_C <- c(rep(22, 4), 24, 24, 6, rep(4,3))

Growth_PAR <- c(50, 70, rep(50, 3), 70, rep(10, 7))

Growth_Photoperiod <- c(rep(12, 4), rep(24, 7))

Media <- c(rep("F2",2), rep("BG11", 2), "BBM", rep("F2", 3), rep("BBM",4))
...
$$
1/σPSII is multiplied by flash spacing (s) to convert the probability that an individual incident photon is absorbed by PSII, into an equivalent photon flux density, i.e. to estimate effective light availability per PSII.
...
Analyses of Variance (ANOVA) were run using the aov function [@rcoreteamLanguageEnvironmentStatistical2023] to detect effects of taxa (Strain), measurement equivalent effective light levels (derived from flash spacing), measurement temperature, and growth salinity, upon the Damping Index.
...
Within, and across, species of *Chlamydomonas* XXXXCITE Pomona PaperXXXX growth under higher salinities maintains F~V~/F~M~ and photosynthetic capacity during stress.  However,  Analyses of Variance (Supplemental Tables) showed no clear influences of growth salinity on the persistence of synchronized S-State cycling across strains of *Chlamydomonas*. It is important to note here that earlier desynchronization of S-State cycling does not indicate a loss or instability of PSII function per se. Instad, desynchronization reflects randomization of the S-States of the PSII population.  Prolonged synchronization under low effective light level is thus an indicator of the capacity of PSII to exploit widely spaced photons under low light, not the stability of PSII function.
...
Salinities vary widely across the habitats of the tested strains, and fluctuate within some habitats during ice formation and melting XXXMACKENZIE CITATIONSXXXX.  Higher salinity stabilizes PSII function under stress across strains of Chlamydomonas XXXXPOMONA CITATIONXXX.  We did not, however,  find influences of growth media salinity on the persistence of synchronized S-State cycling across strains of *Chlamydomonas*. It is important to note that earlier desynchronization of S-State cycling does not indicate a loss nor instability of PSII function *per se*. Instad, desynchronization reflects randomization of the S-States of the PSII population.  Prolonged synchronization under low effective light level is thus an indicator of the capacity of PSII to exploit widely spaced photons under low light, not the stability of PSII function.
