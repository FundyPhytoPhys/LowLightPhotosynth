...
DataIn <- file.path("Data", "RawData", fsep = .Platform$file.sep)
...
SolFitsTrim <- SolFits %>% 
  ...
  select(-c("RFID_User_Data", "Barcode_Data", "Lon", "Lat", "GPS_stat", "StDev...34", "Flag.4", "X","Light_1.1")) %>% # remove superfluous columns
  ...
  separate(Filename, into = c("Project", "YYYYMMDD", "CultureID", "PulseSpace_s", "Ex_WL", "TempCont"), sep = "([\\/\\_])", remove = FALSE) %>%
...
```{r flashnumber vs Fm}
SolFitsTime <- SolFitsTrim2[-c(1:24),] %>%
  ggplot()+
