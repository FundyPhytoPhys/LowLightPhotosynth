...
DataIn <- file.path("..","Data", "RawData","WithDecay","Fit", fsep = .Platform$file.sep)
...
#for NoDecayDataset
SolFitsDecayTrim <- SolFitsDecay %>% 
  filter(!grepl("----", DATE)) %>% # remove rows with "----" 
  select(-c("RFID_User_Data", "Barcode_Data","PIF", "LEDSel", "Dur.Wat", "Lon", "Lat", "GPS_stat","X", "Light_1.1")) %>% # remove superfluous columns
  rename(.cols = c(Fo::Tau3QA))


  mutate(Filename = str_remove(string = Filename, pattern = "_fit.csv")) %>%
  mutate(Filename = str_remove(string = Filename, pattern = "../Data/RawData/WithDecay/Fit/")) %>%
...
# %>%
#   mutate(Sig = as.numeric(as.character(Sig)),
#          Fo = as.numeric(as.character(Fo)),
#          Fm = as.numeric(as.character(Fm)),
#          FvFm = as.numeric(as.character(FvFm)),
#          
...
SolFitsDecayTrim <-SolFitsDecayTrim[-(1:160),] %>%
  group_by(Filename) %>%
  mutate(Sigrunnorm =  Sig/mean(Sig, na.rm = TRUE),
         Forunnorm = Fo/mean(Fo, na.rm = TRUE),
         Fmrunnorm = Fm/mean(Fm, na.rm = TRUE),
         FvFmrunnorm = FvFm/mean(FvFm, na.rm = TRUE)) |>
...
SolFitsTrim %>%
  ggplot()+
  geom_line(aes(x=Flashnumber, y=as.numeric(Fm), group=PulseSpace_s, colour = as.factor(PulseSpace_s))) +
