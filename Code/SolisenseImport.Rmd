...
SolFitsTrim <- SolFits2 %>% 
  filter(!grepl("----", DATE)) %>% # remove rows with "----"
  select(-c("RFID_User_Data", "Barcode_Data", "Lon", "Lat", "GPS_stat","X","Light_1.1")) %>% # remove superfluous columns
  mutate(Filename = str_remove(string = Filename, pattern = "_fit.csv")) %>%
  mutate(Filename = str_remove(string = Filename, pattern = "../Data/RawData/Fit/")) %>%
  #separate(Filename, into=c("Project", "YYYYMMDD", "CultureID", "PulseSpace_s","Ex_WL"), 
          # sep = "([\\/\\_])", remove = FALSE) %>%
...
```{r Flashnumber vs Sig}
SolFitsTrim2 %>%
  ggplot()+
  geom_line(aes(x=Flashnumber, y=as.numeric(Sig), group=Temp_C, colour=Temp_C)) +
  facet_grid("PulseSpace_s") +
  theme_bw()
```
...
Analyzing the time series using the Matrix Profile 
  @vanbenschotenMPANovelCrosslanguage2020

```{r activate packages}
library(tsmp)
```

```{r creating a sample time series data frame}
#for 10c 05S
TS <- SolFitsTrim2[1:24,c(22,18)]
TS$Fo = as.numeric(as.character(TS$Fo))
class(TS$Fo)
```

@bischoffComputesMatrixProfile
```{r building a matrix profile}
matrix <- tsmp(TS, window_size = 4)
```
