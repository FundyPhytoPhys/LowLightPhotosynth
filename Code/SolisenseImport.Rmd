...
#ChlamyMetadata <- "https://docs.google.com/spreadsheets/d/1m8Pjwj4DL10AwdsQo2G3boymSWG1Oq-gJDzjU0csRKs/edit?pli=1#gid=0"
...
```{r parallel SolFitsTrim for Tauvik Data}
#for NoDecay Data
SolFitsTrim <- SolFits %>%
  filter(!grepl("----", DATE)) %>% # remove rows with "----" 
...
**Unresolved**
20230925_SeGu1005_2C_8s   - 2 full runs
20230927_SeGu1008_2C_16s  - contains 2 full runs
20230926_SeGu1001_6C_16s  - last 32 are a run, but spacing is 19s?
...
20230927_SeGu1008_0C_16s  - remove first flash
...
20230926_SeGu1008_10C_16s - keep last 32
20230927_SeGu1001_10C_16s - keep last 32
20230926_SeGu1008_6C_16s  - keep last 32
20230927_SeGu1008_6C_16s  - keep last 32


```{r manual trim Takuvik}
SolFitsTrim_last32 <- SolFitsTrim %>%
   filter(!Filename %in% c('Takuvik/20230925_SeGu1005_2C_16s', 'Takuvik/20230926_SeGu1001_6C_16s', 'Takuvik/20230927_SeGu1008_2C_16s','Takuvik/20230925_SeGu1005_2C_8s')) %>% 
  group_by(Filename) %>%
...
#SolFitsTrimSeGu1005_2C_8s <- SolFitsTrim %>%
 # filter(Filename == 'Takuvik/20230925_SeGu1005_2C_8s') %>%
 # slice() %>% 
 # mutate(Flashnumber2 = row_number()) 

#SolFitsTrimSeGu1008_2C_16s <- SolFitsTrim %>%
 # filter(Filename == '20230927_SeGu1008_2C_16s') %>%
 # slice() %>% 
 # mutate(Flashnumber2 = row_number())
  
#SolFitsTrim2 <- rbind(SolFitsTrim_last32, SolFitsTrimSeGu1005_2C_16s,SolFitsTrimSeGu1005_2C_8s, SolFitsTrimSeGu1008_2C_16s) 
```
...
Link ID with Species 
Mutate to add species in SolFits Trim 
Graph group_by Taxon
**Issue: Many ys per x need to somehow assign names to IDs**
```{r merge data}
SolFitsTrim2 <- SolFitsTrim2 %>%
  rename("ID"= "CultureID")%>%
  inner_join(TakuvikMetadata[,c("ID", "Strain")], by = "ID")
```
...
SolFitsTrim20230918_MaPo1001  |>
  ggplot()+
  geom_line(aes(x=Flashnumber2, y=FvFm, colour = as.factor(PulseSpace_s))) +
  facet_grid(rows = vars(PulseSpace_s, Temp_C), cols = vars(CultureID)) +
  labs(title = TargetTaxa) +
  theme_bw()
...
saveRDS(SolFitsTrim2, file.path(DataOut, paste(Project,  TargetTaxa, "SolFitsTrim.Rds", sep = "_"), fsep = .Platform$file.sep))
```
