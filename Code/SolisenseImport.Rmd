...
DataIn <- file.path("..","Data", "RawData","Chlorella", "NoDecay", fsep = .Platform$file.sep)
DataRefDecay <- file.path("..","Data", "RawData","Chlorella", "RefDecay", fsep = .Platform$file.sep)
...
SolFitsTrim <- SolFitsTrim |>
 ...
 # mutate(Filename = str_remove(string = Filename, pattern = "LOW_")) %>%
  separate(Filename, into=c("Species", "Type", "YYYYMMDD", "CultureID", "Temp_C", "PulseSpace_s"), sep = "([\\/\\_])", remove = FALSE) %>%
...
SolFitsTrim %>%
  ggplot()+
  geom_line(aes(x=Flashnumber, y=Fmrunnorm, group=PulseSpace_s, colour = as.factor(PulseSpace_s))) +
...
SolFitsTrim %>%
  ggplot()+
  geom_line(aes(x=Flashnumber, y= Fmrunnorm, group=Temp_C, colour=Temp_C)) +
  scale_x_continuous(limits=c(0,20), breaks=seq(0,20,4)) +
