...
sstate_damping <- readRDS("~/LowLightPhotosynth/Data/ProcessedData/LOW_SState_Damping.Rds") %>%
  mutate(Strain = factor(Strain), 
         #PulseSpace_s = factor(PulseSpace_s), 
  ...
  Strain.Label = ifelse(Strain == "Chlamydomonas_ICEMDV", "~italic('C. ICEMDV')", 
                        ifelse(Strain == "Chlamydomonas_malina","~italic('C. malina')",
                        ifelse(Strain == "Chlamydomonas_priscuii", "~italic('C. priscuii')",
                        ifelse(Strain == "Chlamydomonas_reinhardtii", "~italic('C. reinhardtii')",
                        ifelse(Strain == "Chlorella_vulgaris", "~italic('C. vulgaris')",
                        ifelse(Strain == "Fragilariopsis_cylindrus", "~italic('F. cylindrus')", 
                        ifelse(Strain == "Thalassiosira_pseudonana", "~italic('T. pseudonana')",
                               NA)))))))) 
...
diatom.vlines <- data.frame(
  Strain.Label = c("~italic('F. cylindrus')", "~italic('F. cylindrus')", "~italic('T. pseudonana')"),
  Temp.Lab = c("Grown~at~0*degree*C", "Grown~at~6*degree*C","Grown~at~22*degree*C"), 
  GrowthTemp_C = c(0.1, 6, 22))

algae.vlines <- data.frame(
  Strain.Label = c("~italic('C. ICEMDV')",  "~italic('C. malina')", "~italic('C. priscuii')", 
...
```{r computing flash spacing averages for secondary axes of heatmaps}
spacing.diatom <- doBy::summaryBy(Light_Level ~ PulseSpace_s, data = sstate_damping, 
                   subset = Strain %in% c("Fragilariopsis_cylindrus", "Thalassiosira_pseudonana"))

spacing.algae <- doBy::summaryBy(Light_Level ~ PulseSpace_s, data = sstate_damping, 
                   subset = Strain %in% c("Chlamydomonas_ICEMDV", "Chlamydomonas_malina", 
                             "Chlamydomonas_priscuii", "Chlorella_vulgaris", "Chlamydomonas_reinhardtii"))
```
...
mtrx.data  %>%
  ...
  labs(x = "Measurement Temperature (°C)", 
       y= "Flash Spacing (s)") +
  ggh4x::facet_nested_wrap(vars(ThermalTolerance, Strain.Label), ncol=4, nrow=2, scales="free_x",
...
predict.diff.gam <- function(damping) {
  ...
  data <- data %>%
    ...
    mutate(Strain = factor(unique(damping$Strain)), 
           Strain.Label = unique(damping$Strain.Label),
...
predict.gam.d %>%
  ...
  ggh4x::facet_nested_wrap(vars(ThermalTolerance, Strain.Label), ncol = 1, strip.position = "right",
                             labeller = label_parsed)+
  theme_thesis() +
...
predict.gam.t <- bind_rows(predict.gam.t) %>%
  mutate(Temp.Lab = paste0("Grown~at~", GrowthTemp_C,"*degree*C"), 
         Temp.Lab )
```
...
predict.gam.t %>%
  ...
  scale_y_continuous(expand=c(0,0), sec.axis = dup_axis(
    breaks = c(spacing.diatom$Light_Level.mean), labels = c(spacing.diatom$PulseSpace_s),
    name = sym("Equivalent Flash Spacing (s)"))) +
  ggh4x::facet_nested_wrap(vars(Strain.Label, Temp.Lab), ncol = 2, 
...
predict.gam.t %>%
  ...
  scale_y_continuous(expand=c(0,0),sec.axis = dup_axis(
    breaks = c(spacing.algae$Light_Level.mean), labels = c(spacing.algae$PulseSpace_s),
    name = sym("Equivalent Flash Spacing (s)"))) +
  ggh4x::facet_nested_wrap(vars(ThermalTolerance, Strain.Label), ncol = 3, strip.position = "top",
                       scales = "free_x", 
                       labeller = label_parsed) +
