...
Cp_data %>%
  ...
  ggplot() +
  geom_point(aes(x=Flashnumber, y=FvFmrunnorm, col=as.factor(SState))) +
  scale_color_manual(values = c(carto_pal(n=4,"Temps"))) +
  labs(x= "Flash Number", 
...
diff.sstate <- split(sstate_damping, sstate_damping$Strain)
names(diff.sstate) <- c("Ci_damping", "Cm_damping", "Cp_damping", "Cr_damping", 
                         "Cv_damping", "Fc_damping", "Tp_damping")

list2env(diff.sstate, envir = .GlobalEnv)
```


```{r Prepare damping list }
#Split Fc by Growth Temp 
FcG0_damping <- diff.sstate[["Fc_damping"]] %>% filter(GrowthTemp_C == 0)
FcG6_damping <- diff.sstate[["Fc_damping"]] %>% filter(GrowthTemp_C == 6)

temp.sstate <- list(
  Ci_damping = Ci_damping, Cm_damping = Cm_damping, Cp_damping = Cp_damping,
  Cr_damping = Cr_damping, Cv_damping = Cv_damping,
  Tp_damping = Tp_damping, FcG0_damping = FcG0_damping, FcG6_damping = FcG6_damping
)
```
...
ggsave("pres_mtrx.png", width = 10 , height = 6, dpi = 900,
       plot=last_plot(),
       path="~/Downloads")
...
mtrx.data  %>%
  ...
  labs(x = "Measurement Temperature (°C)", 
       y= "Flash Spacing (s)") +
  facet_wrap(vars(GrowthTemp_C), labeller = labeller(GrowthTemp_C = temp.labs)) +
  theme_thesis() +
  theme(legend.title = element_text(size=11))
...
### Deviation From Growth Temp

```{r Functions for GAM by Diff}
predict.diff.gam <- function(damping) {
...
```{r}
summary.gam.d[["Ci_damping"]][["summary"]]
```
...
# ggsave("diatoms_diff.png", width = 7 , height = 5, dpi = 900,
#        plot=last_plot(),
#        path="../Output")
```
...
predict.gam.d %>%
  ...
  ggh4x::facet_nested_wrap(vars(ThermalTolerance, Strain), ncol = 2, strip.position = "right",
                           labeller = labeller(Strain=strain.labs)) +
  theme_thesis() +
  theme(strip.text = element_text(face="italic"))
...
# ggsave("diatoms_temp.png", width = 7 , height = 4.5, dpi = 900,
#        plot=last_plot(),
#        path="../Output")
```
...
predict.gam.t %>%
  ...
  labs(x = "Light Level", y = "Damping Index") +
...
# ggsave("diatom_comp.png", width = 8 , height = 3.5, dpi = 900,
#        plot=last_plot(),
#        path="../Output")
```
...
predict.gam.t %>%
  ...
  labs(x = "Light Level", y = "Damping Index") +
  scale_color_manual(values=c(carto_pal(n=5,"Temps")), labels=c(strain.labs)) +
  scale_shape_manual(values=c(16,3,17,18,8), labels=c(strain.labs)) +
  theme_thesis() +
  theme(legend.text = element_text(face = "italic")) 

# ggsave("algae_comp.png", width = 8 , height = 3.5, dpi = 900,
#        plot=last_plot(),
#        path="../Output")
```
...
predict.diff.group <- function(damping) {
  ...
  Group <- unique(damping$Group)
  
  gam <- mgcv::gam(DampingIndex ~  s(TempDiff_C, k=3) + s(Light_Level) + 
                    ti(TempDiff_C, Light_Level, k=3), data = damping)

  data <- with(damping, expand.grid(
    Light_Level = seq(0.047, 0.53, 0.01),
  ...
  data <- data %>%
    mutate(DampingIndex = ifelse(DampingIndex < 0, 0, DampingIndex)) %>%
    mutate(Group = Group) %>%
    separate(Group,into= c("ThermalTolerance", "Type"), sep="_")
...
```{r}
predict.group <- lapply(damping.group, predict.diff.group)
...
```{r warning=FALSE}
predict.group %>%
  ggplot(aes(x=TempDiff_C, y=Light_Level, fill= DampingIndex)) +
  geom_tile() + 
  labs(x= "Deviation from Growth Temperature (°C)",
       y= expression("Light Level ("*mu*"mol photons m"^"-2"~s^-1*")"),
       fill= "Damping Index") +
  #facet_grid(rows=vars(ThermalTolerance), cols=vars(Type)) +
  #geom_vline(xintercept= 0, col= "white", linewidth= 0.7,  linetype = "dashed") +
  #scale_fill_gradientn(colors = pals::ocean.ice(26600)) +
...
predict.group %>%
  ...
  scale_y_continuous(expand=c(0,0)) +
  facet_grid(rows=vars(ThermalTolerance), cols=vars(Type), scales = "free_x") +
  theme_pres() 
...
```{r}
predict.group.temp <- lapply(damping.group, predict.temp.group)
predict.group.temp <- bind_rows(predict.group.temp) 
```
...
predict.group.temp %>%
  ...
  labs(x= "Measurement Temperature (°C)",
       y= expression("Light Level ("*mu*"mol photons m"^"-2"~s^-1*")"),
       fill= "Damping Index") +
  facet_grid(rows=vars(Type), cols=vars(ThermalTolerance), scales = "free_x") +
  #geom_vline(xintercept= 0, col= "white", linewidth= 0.7,  linetype = "dashed") +
  #scale_fill_gradientn(colors = pals::ocean.ice(26600)) +
  scale_fill_viridis(option = "viridis") +
  scale_x_continuous(expand=c(0,0)) + scale_y_continuous(expand=c(0,0)) +
...
```{r}
test <- mgcv::gam(DampingIndex ~  s(TempDiff_C, k=3) + s(Light_Level) + 
                    ti(TempDiff_C, Light_Level, k=3), data = Cp_damping)

summary(test)
vis.gam(test, plot.type= "contour")
gam.check(test)
```
...
# predict.diff.loess <- function(damping) {
#   
#   Strain <- unique(damping$Strain)
#   
...
# predict.loess.d <- lapply(diff.sstate, predict.diff.loess)
# predict.loess.d <- bind_rows(predict.loess.d) 
```

Measurement Temperature
```{r set function by temp}
# predict.temp.loess <- function(damping) {
#   
#   Strain <- unique(damping$Strain)
#   GrowthTemp_C <- unique(damping$GrowthTemp_C)
#   
...
```{r by temp df setup}
# predict.loess.t <- lapply(temp.sstate, predict.temp.loess)
# predict.loess.t <- bind_rows(predict.loess.t) 
```

- Plots are the same as in GAM section 
