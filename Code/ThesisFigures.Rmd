...
frag.data.loess <- loess(DampingIndex ~ TempDiff_C * PulseSpace_s, data = frag)
frag.xgrid <-  seq(-6, 10, 0.3)
frag.ygrid <-  seq(1, 16, 0.3)
frag.data.fit <-  expand.grid(TempDiff_C = frag.xgrid, PulseSpace_s = frag.ygrid)
frag.mtrx3d <-  predict(frag.data.loess, newdata = frag.data.fit)

frag.mtrx.melt <- melt(frag.mtrx3d, id.vars = c('TempDiff_C', 'PulseSpace_s'), measure.vars = 'DampingIndex')
names(frag.mtrx.melt) <- c('TempDiff_C', 'PulseSpace_s', 'DampingIndex')

# Return data to numeric form
frag.mtrx.melt$TempDiff_C <- as.numeric(str_sub(frag.mtrx.melt$TempDiff_C, str_locate(frag.mtrx.melt$TempDiff_C, '=')[1,1] + 1))
frag.mtrx.melt$PulseSpace_s <-  as.numeric(str_sub(frag.mtrx.melt$PulseSpace_s, str_locate(frag.mtrx.melt$PulseSpace_s, '=')[1,1] + 1))

frag.fig <- plot_ly(frag.mtrx.melt, x = ~TempDiff_C, y = ~PulseSpace_s, z = ~DampingIndex, type = "contour",
