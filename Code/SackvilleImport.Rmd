...
SolFitsTrim_WI23 <- SolFitsTrim_WI23 %>%
  mutate(Filename = str_remove(string = Filename, pattern = "_fit.csv")) %>%
  mutate(Filename = str_remove(string = Filename, pattern = "../Data/RawData/Sackville/Pilot/")) %>%
  separate(Filename, into=c("Species","YYYYMMDD", "CultureID", "Temp_C", "PulseSpace_s"), sep = "([\\/\\_])", remove = FALSE) %>%
  mutate(YYYYMMDD = lubridate::ymd(YYYYMMDD),
          TIME = as.character(TIME)) %>%
   rename(ObsDate = DATE,
...
#to check spacing to find correct runs 
 SolFitsTest <- SolFitsTrim %>%
   filter(Filename == "Tpseudonana/20230912_NaOm1663_18C_8S") %>%
...
SolFitsTrim2 <- SolFitsTrim2 %>%
  filter(!Filename == "Tpseudonana/20230911_NaOm1663_22C_8S")
```
