...
Cr:
MaPo2002_28C_1 s, another run appended? normalization?

Ce
MaPo2020_16C_1s; another run appended?

Cm
nothing obvious
MaPo2010_4C_8s another run appended?

Ck
nothing obvious

Cp
some drift nothing obvious

```{r problemplot_filter}
SolFitsMeta <- SolFitsMeta |>
  filter(!grepl("MaPo2020_16C_1s", Filename)) |> #remove for now; better to truncate length
  filter(!grepl("MaPo2002_28C_1s", Filename)) |>
  filter(!grepl("MaPo2019_16C_2s", Filename)) |> #remove for now; better to truncate length
  filter(!grepl("MaPo2010_4C_8s", Filename))

#|>
 # filter(Strain != "Chlamydomonas_reinhardtii" & Temp_C != 12) #poor quality runs
