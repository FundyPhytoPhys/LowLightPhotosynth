...
SolFitsModels <- LOW_SolFits |>
  ...
  mutate(FmSStates_model = map(data, ~nlsLM(.$Fmrunnorm ~ exp_eqn(x = .$Flashnumber, Intercept, mu),
                                      data = .x,
...
SolFitsModelsPlateau <- LOW_SolFits |>
  nest(.by = c("Filename", "Temp_C", "PulseSpace_s", "SState")) |>
  mutate(FmSStates_model = map(data, ~nlsLM(.$Fmrunnorm ~ expplateau_eqn(x = .$Flashnumber, Intercept, Plateau = 1,  mu),
                                      data = .x,
                                      start = list(Intercept = 1.1, mu = 0.01),
                                      control = nls.lm.control(maxiter = 100)
                                      )
