...
LOW_SolFits %>%
  ggplot() +
  geom_point(aes(x = Flashnumber, y = log(FvFmrunnorm), colour = as.factor(SState))) +
...
Simulate |>
  ...
  geom_point(aes(x = Flashnumber, y = FM, colour = as.factor(SignifFlash))) +
...
for(i in c(1:19)) {
  ...
  OptRecomb = min(OutputVector, na.rm = TRUE)
  OptRecomb
}
```
