...
SolFitsTrim2 <- SolFitsTrim2 %>%
  group_by(Filename) %>%
  mutate(Sigmean = mean(Sig[(length(Sig) - 10):length(Sig)], na.rm = TRUE),
         Fomean = mean(Fo[(length(Fo) - 10):length(Fo)], na.rm = TRUE),
         Fmmean = mean(Fm[(length(Fm) - 10):length(Fm)], na.rm = TRUE),
...
SolFitsPriscui <- conversion(SolFitsPriscui) 
...
saveRDS(SolFitsPriscui, file.path(DataOut, paste(Project, "Chlamydomonas", "priscui", "SolFitsTrim.Rds", sep = "_"), fsep = .Platform$file.sep))
