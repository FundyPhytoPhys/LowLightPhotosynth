...
```{r}
SolFitsTrim3 <- conversion(SolFitsTrim2)
```

```{r add deviation from growth temp}
SolFitsTrim3 <- SolFitsTrim3 %>%
  mutate(GrowthTemp_C = ifelse(CultureID %in% c("MaPo1001", "MaPo1002", "MaPo1003", "MaPo1004"), 4, ifelse(CultureID %in% c("MaPo1005", "MaPo1006"), 24, NA))) %>%
...
### Group by Taxon 
```{r}
SolFitsPriscuii <- SolFitsTrim3 %>%
  filter(CultureID %in% c("MaPo1001", "MaPo1004"))

SolFitsICEMDV <- SolFitsTrim3 %>%
  filter(CultureID %in% "MaPo1002")

SolFitsMalina <- SolFitsTrim3 %>%
  filter(CultureID %in% "MaPo1003")

SolFitsReinhardtii <- SolFitsTrim3 %>%
