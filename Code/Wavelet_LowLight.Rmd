...
reconstructwave <- function(wavelet_list, siglvl = siglvl_reconstruct) {
...
### Fc
```{r import Fc data}
FitFilesFc <- list.files(path = DataIn, pattern = paste(Fc,FileID, sep = "_"), full.names = TRUE) #Fc henceforth refers to Fragilariopsis cylindrus

unique(duplicated(FitFilesFc)) #test for duplicate file names

SolFitsFc <- FitFilesFc %>%
  map_df(~read.RDS(flnm = .)) 
```

```{r condense Fc trials}
Fcdata <- summaryBy(FvFmrunnorm ~ Temp_C + PulseSpace_s + Flashnumber, data=SolFitsFc, FUN=c(mean)) %>% 
  rename(FvFmrunnorm = FvFmrunnorm.mean) %>%
  mutate(Condition=paste0(Temp_C,"C",PulseSpace_s,"s")) %>%
...
### Ci
```{r import Ci data }
FitFilesCi <- list.files(path = DataIn, pattern = paste(Ci,FileID, sep = "_"), full.names = TRUE)

unique(duplicated(FitFilesCi)) #test for duplicate file names

SolFitsCi <- FitFilesCi %>% 
  map_df(~read.RDS(flnm = .))
```

```{r condense Ci trials}
Cidata <- summaryBy(FvFmrunnorm ~ Temp_C + PulseSpace_s + Flashnumber, data=SolFitsCi, FUN=c(mean)) %>%
  rename(FvFmrunnorm = FvFmrunnorm.mean) %>%
  mutate(Condition=paste0(Temp_C,"C",PulseSpace_s,"s")) %>%
  split_by("Condition", drop=FALSE)
```
...
### Cm 
```{r import Cm data}
FitFilesCm <- list.files(path = DataIn, pattern = paste(Cm,FileID, sep = "_"), full.names = TRUE)

unique(duplicated(FitFilesCm)) #test for duplicate file names

SolFitsCm <- FitFilesCm %>% 
  map_df(~read.RDS(flnm = .)) 
```

```{r condense Cm trials}
Cmdata <- summaryBy(FvFmrunnorm ~ Temp_C + PulseSpace_s + Flashnumber, data=SolFitsCm, FUN=c(mean)) %>% 
  rename(FvFmrunnorm = FvFmrunnorm.mean) %>%
  mutate(Condition=paste0(Temp_C,"C",PulseSpace_s,"s")) %>%
  split_by("Condition", drop=FALSE)
```
...
### Cp
```{r import Cp data}
FitFilesCp <- list.files(path = DataIn, pattern = paste(Cp,FileID, sep = "_"), full.names = TRUE)

unique(duplicated(FitFilesCp)) #test for duplicate file names

SolFitsCp <- FitFilesCp %>%
  map_df(~read.RDS(flnm = .))
```

```{r condense Cp trials}
Cpdata <- summaryBy(FvFmrunnorm ~ Temp_C + PulseSpace_s + Flashnumber, data=SolFitsCp, FUN=c(mean)) %>%
  rename(FvFmrunnorm = FvFmrunnorm.mean) %>%
  mutate(Condition=paste0(Temp_C,"C",PulseSpace_s,"s")) %>%
  split_by("Condition", drop=FALSE)
```
...
### Cr
```{r import Cr data}
FitFilesCr <- list.files(path = DataIn, pattern = paste(Cr,FileID, sep = "_"), full.names = TRUE)

unique(duplicated(FitFilesCr)) #test for duplicate file names

SolFitsCr <- FitFilesCr %>% 
  map_df(~read.RDS(flnm = .)) 
```

```{r condense Cr trials}
Crdata <- summaryBy(FvFmrunnorm ~ Temp_C + PulseSpace_s + Flashnumber, data=SolFitsCr, FUN=c(mean)) %>% 
  rename(FvFmrunnorm = FvFmrunnorm.mean) %>%
  mutate(Condition=paste0(Temp_C,"C",PulseSpace_s,"s")) %>%
  split_by("Condition", drop=FALSE)
```
...
### Cv
```{r import clean  data}
FitFilesCv <- list.files(path = DataIn, pattern = paste(Cv, FileID, sep = "_"), full.names = TRUE) 
...
### Tp
```{r import Tp data}
FitFilesTp <- list.files(path = DataIn, pattern = paste(Tp ,FileID, sep = "_"), full.names = TRUE) 
...
sstate_damping <- rbind(Fc_damping, Ci_damping, Cm_damping, Cp_damping, Cr_damping, Cv_damping, Tp_damping) %>%
  ...
  separate(condition, into=c("Temp_C", "PulseSpace_s"), sep = "C") %>%
  mutate(PulseSpace_s = str_remove(PulseSpace_s, "s")) %>%
  rename(DampingIndex = damping.index) 
```

```{r saving as RDS}
#save as RDS to be able to work on figures without running all the lengthy wavelet transformation 
