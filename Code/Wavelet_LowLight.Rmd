...
### Fc grown at 0C
```{r import Fc G0 data}
FcG0_data <- SolFitsFragilariopsisG0 #list.files(path = DataIn, pattern = paste(Fc,"G0", FileID, sep = "_"), full.names = TRUE) %>%
 # readRDS()

FcG0_data <- summaryBy(FvFmrunnorm ~ Light_Level + Temp_C + Flashnumber, data=FcG0_data, FUN=mean, keep.names = TRUE) %>%
  mutate(Condition = paste0(Temp_C,"C","_",Light_Level,"umol")) %>%
  split_by("Condition")
```
...
FcG0_damping <- damping(FcG0_reconstruction) %>% 
  ...
  separate(condition, into=c("Temp_C", "Light_Level"), sep = "_") %>%
  mutate(Light_Level = str_remove(Light_Level, "umol")) %>%
  mutate(Temp_C = str_remove(Temp_C, "C")) %>%
  rename(DampingIndex = damping.index) %>%
  mutate(TempDiff_C = Temp_C)
```

```{r}
FcG0_damping %>%
  ggplot() + 
  geom_tile(aes(x = Temp_C, y = as.factor(Light_Level), fill = DampingIndex)) +  
...
FcG6_data <- summaryBy(FvFmrunnorm ~ Temp_C + PulseSpace_s + Flashnumber, data=FcG6_data, FUN=mean, keep.names = TRUE) %>%
...
FcG6_damping <- damping(FcG6_reconstruction) %>% 
  ...
  mutate(Temp_C = str_remove(Temp_C, "C")) %>%
  rename(DampingIndex = damping.index) %>%
  mutate(TempDiff_C = as.numeric(Temp_C) - 6)
```
...
Ci_damping <- damping(Ci_reconstruction) %>% 
  ...
  mutate(Temp_C = str_remove(Temp_C, "C")) %>%
  rename(DampingIndex = damping.index) %>%
  mutate(TempDiff_C = as.numeric(Temp_C) - 4)
```
...
Cr_damping <- damping(Cr_reconstruction) %>%
  ...
  mutate(Temp_C = str_remove(Temp_C, "C")) %>%
  rename(DampingIndex = damping.index) %>%
  mutate(TempDiff_C = as.numeric(Temp_C) - 24)
```
...
Cv_data <- #list.files(path = DataIn, pattern = paste(Cv, FileID, sep = "_"), full.names = TRUE) %>%
   readRDS("../Data/CleanData/LOW_Chlorella_vulgaris_SolFitsTrim.Rds")
...
Cv_damping <- damping(Cv_reconstruction) %>%
  ...
  mutate(Temp_C = str_remove(Temp_C, "C")) %>%
  rename(DampingIndex = damping.index) %>%
  mutate(TempDiff_C = as.numeric(Temp_C) - 18)
```
...
Tp_data <- #list.files(path = DataIn, pattern = paste(Tp ,FileID, sep = "_"), full.names = TRUE)  %>%
  readRDS("../Data/CleanData/LOW_Thalassiosira_pseudonana_SolFitsTrim.Rds")
