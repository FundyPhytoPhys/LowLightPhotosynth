...
Fc_data <- summaryBy(FvFmrunnorm ~ PulseSpace_s + Temp_C + Flashnumber + Light_Level + TempDiff_C, data=Fc_fits, FUN=mean, keep.names = TRUE) %>%
  splitBy(formula = ~ PulseSpace_s + Temp_C + Light_Level + TempDiff_C) %>%
...
Fc_damping <- damping(Fc_reconstruction)  %>%
  ...
  separate(condition,into= c("Strain", "PulseSpace_s", "Temp_C", "Light_Level", "TempDiff_C"), sep="_") %>%
  mutate(Strain = Fc) %>%
  relocate(DampingIndex, .before = TempDiff_C)
```
