...
# Introduction
This RMD computes a wave transformation and extracts the duration of fluorescence oscillations for the cleaned data generated from OttawaImport.

It generates outputs from all growth salinities, and at growth salinities filtered for optimal growth.
...
Chlamy_data <- summaryBy(FvFmrunnorm ~ Strain + PulseSpace_s + Temp_C + TempDiff_C +  Flashnumber + LightLevel_ue + NaCl_mM + NaCl_opt + GrowthTemp_C + Year, data=SolFitsChlamyMeta, FUN=mean, keep.names = TRUE) %>%
  splitBy(formula = ~ Strain + PulseSpace_s + Temp_C + TempDiff_C + LightLevel_ue + NaCl_mM + NaCl_opt + GrowthTemp_C + Year)
...
Chlamy_damping <- damping(Chlamy_reconstruction)  %>%
  ...
  separate(condition, into=c("Strain", "PulseSpace_s", "Temp_C", "TempDiff_C", "LightLevel_ue", "NaCl_mM", "NaCl_opt", "GrowthTemp_C", "Year"), sep = "\\|") |>
  filter(!is.na(PulseSpace_s)) |>  # fix a  bad row C.priscui; not working?
  mutate(Temp_C = as.numeric(Temp_C),
         ...
         GrowthTemp_C = as.numeric(GrowthTemp_C)
         )
...
Chlamy_damping |>
  ...
  geom_tile(aes(y = PulseSpace_s, x = Temp_C, colour = DampingIndex, fill = DampingIndex)) +
  scale_y_reverse() + 
  facet_grid(cols = vars(NaCl_mM),   rows = vars(Strain, Year)) +
  theme_bw()
```
...
saveRDS(Chlamy_damping, file.path(DataOut, paste(Project, "Chlamy_dampingYear.Rds", sep = "_"), fsep = .Platform$file.sep))
```

```{r}

saveRDS(Chlamy_wavelets, file.path(DataOut, paste(Project, "Chlamy_waveletsYear.Rds", sep = "_"), fsep = .Platform$file.sep))
```

```{r}

saveRDS(Chlamy_reconstruction, file.path(DataOut, paste(Project, "Chlamy_reconstructionsYear.Rds", sep = "_"), fsep = .Platform$file.sep))
```
