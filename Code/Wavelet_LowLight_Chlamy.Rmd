...
Chlamy_data <- summaryBy(FvFmrunnorm ~ Strain + PulseSpace_s + Temp_C + Flashnumber + LightLevel_ue + NaCl_mM, data=SolFitsChlamyMeta, FUN=mean, keep.names = TRUE) %>%
  splitBy(formula = ~ Strain + PulseSpace_s + Temp_C + LightLevel_ue + NaCl_mM)



# %>%
#   setNames(., paste0(gsub("\\|", "_", names(.))))

#Need to fix if going to run multiple strains simultaneously...
```
...
Chlamy_damping <- damping(Chlamy_reconstruction)  %>%
  ...
  separate(condition, into=c("Strain", "PulseSpace_s", "Temp_C", "LightLevel_ue", "NaCl_mM"), sep = "\\|") |>
